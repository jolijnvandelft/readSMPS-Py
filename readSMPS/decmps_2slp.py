...
class decompose:
    ...
    def create_LSsub_constr(self, obs, incmbt):
        ...
        for c in constr:
            ...
            for v in copied_sub_model_vars:
                ...
                constr_in_copy = copied_mean_model.getConstrByName(
            ...
            for v in range(len(self.prob.master_vars)):
                if "eta" not in self.prob.master_vars[v].getAttr("VarName"):
                    master_var_in_copy = copied_mean_model.getVarByName(
                        self.prob.master_vars[v].getAttr("VarName")
                    )
                    Cx += (
                        copied_mean_model.getCoeff(constr_in_copy, master_var_in_copy)
                        * incmbt[v]
                    )

            copied_sub_model.addConstr(
    ...
    def create_master_multi(self, scen_num):
        ...
        self.prob.master_vars = self.prob.master_model.getVars()
        varName = [j.getAttr("VarName") for j in self.prob.master_vars]
        eta = self.prob.master_model.addVars(
            range(scen_num),
            lb=0.0,
            ub=gb.GRB.INFINITY,
            obj=1.0,
